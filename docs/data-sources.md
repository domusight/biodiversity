...
| OS Open Map Local | `SurfaceWater_Line` in **Surface water lines** (streams too narrow to be polygons). `SurfaceWater_Area` in **Surface water area**. `TidalWater` in **Tidal water**. `Woodland` in **Woodland polygons**. | Open Government Licence. [OS Data Hub](https://osdatahub.os.uk/downloads/open/OpenMapLocal). [SurfaceWater_Line](https://docs.os.uk/os-downloads/products/maps-and-imagery-portfolio/os-openmap-local/os-openmap-local-technical-specification/feature-types/surfacewater_line). |
...
| OS Open Rivers | Leave **River centrelines** empty. This is a connected network. `fictitious` means a straight line, not a culvert, and the open product does not mark underground links. Use `SurfaceWater_Line` instead. | Open Government Licence. [OS Data Hub](https://osdatahub.os.uk/downloads/open/OpenRivers). [WatercourseLink](https://docs.os.uk/os-downloads/products/water-portfolio/os-open-rivers/os-open-rivers-technical-specification/feature-types/watercourselink). |
...
- Stream width is not surveyed. `SurfaceWater_Line` is buffered by the assumed width, default 8 m. Wider water keeps the `SurfaceWater_Area` polygon.
